...
public class AnnotatedBeanDefinitionReader {
	 ...
	 * @since 3.1
	 *
	 * 委托AnnotationConfigUtils来创建reader
	 */
	...
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		// 把类的作用域添加到bean定义这个数据结构中
		abd.setScope(scopeMetadata.getScopeName());

		// 通过beanNameGenerator生成bean的名字
		String beanName = (name != null ? name : this.beanNameGenerator.generateBeanName(abd, this.registry));

		// 处理Lazy,DependsOn,Primary,Role等注解   如果有注解，将他们添加到abd这个bean定义的数据结构（BeanDefinition）中
		AnnotationConfigUtils.processCommonDefinitionAnnotations(abd);

		/**
		 * 处理其他注解，比如qualifier，这里qualifiers如果是正常调用的话，永远是空，因为spring源码写死了传入的时候就是null,除非你能手动调用这个方将其传入。之所以这样写是这一段代码spring不打算让用户来使用，而是spring在内部自己调用的
		 */
		if (qualifiers != null) {
		...
		/**
		 * BeanDefinitionHolder也是一个bean定义的数据结构，只不过他比之前的AnnotatedGenericBeanDefinition内容更丰富
		 * 它包含了原来的abd中的信息，还包含了一个beanName
		 * 这个数据结构的内部可以理解为一个map
		 */
		...
		/**
		 * 将definitionHolder注册到registry这个里面
		 * registry中维护了一个map<beanName, definitionHolder>和一个List<beanName>
		 *
		 * 到此为止，我们就将bean的定义读取完成了
		 *
		 */
